...
export const eateryDialog = (clickedEatery) => {
  return ` 
        <dialog id="eateryDialog">
            ...
            <div id="wheelchair">♿WheelChair: ${boolConvert(
              clickedEatery.ameneties.wheelchairAccessible
            )}</div>
            <div id="petFriendly">🐕Pet Friendly: ${boolConvert(
              clickedEatery.ameneties.petFriendly
            )}</div>
            <div id="wifi">📶Wifi: ${boolConvert(
              clickedEatery.ameneties.wifi
            )}</div>
            <div id="diaperFacility">👶Diaper Facility: ${boolConvert(
              clickedEatery.ameneties.diaperFacility
            )}</div>
            <div id="playground">🧒 👧Playground: ${boolConvert(
              clickedEatery.ameneties.playground
            )}</div>
            <div id="restrooms">🚻Public Restrooms: ${boolConvert(
              clickedEatery.ameneties.restrooms
            )}</div>
            </div>
